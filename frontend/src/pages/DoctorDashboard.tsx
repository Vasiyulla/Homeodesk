...
const DoctorDashboard: React.FC<DoctorDashboardProps> = ({ title }) => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      if (apptsResult.success && apptsResult.data) {
        // Filter for patients currently in waiting room today, sort emergencies first
        const arrived = apptsResult.data
  ...
  const { isConnected } = useWebSocket('waiting-room', (msg) => {
    if (msg.event === 'patient_checked_in') {
      info('Patient Arrived', 'A new patient has entered the waiting room');
      fetchDashboardData();
    }
  });
  ...
  const pendingCasesCount = cases.filter(c => c.status === 'DRAFT' || c.status === 'WAITING_FOR_DOCTOR').length;
      ...
      >
        ...
        <motion.div variants={itemVariants} className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 md:gap-6 bg-white/60 p-5 md:p-8 rounded-2xl md:rounded-3xl border border-white shadow-glass-sm relative overflow-hidden">
          ...
          <div className="relative z-10 w-full">
            ...
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-brand-100 text-brand-700 text-[10px] md:text-xs font-bold mb-3 uppercase tracking-wider">
              {user?.organization_clinic_type || 'Hospital'} Workspace
            </div>
        ...
        <motion.div variants={itemVariants} className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-6">
          ...
          <Card 
            className="!p-4 md:!p-6 flex flex-col md:flex-row items-start md:items-center gap-3 md:gap-5 border-l-4 border-l-amber-500 bg-gradient-to-br from-amber-50 to-white cursor-pointer hover:shadow-md transition-shadow col-span-2 md:col-span-1"
          ...
          >
            ...
            <div>
              ...
              <h3 className="text-2xl md:text-3xl font-bold text-surface-900">{pendingCasesCount}</h3>
            </div>
          </Card>
        </motion.div>
                  ...
                  {waitingPatients.map((appt: Appointment) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className={`bg-white rounded-xl p-4 shadow-sm relative overflow-hidden transition-all ${appt.is_emergency ? 'border-2 border-red-500 shadow-red-500/20' : 'border border-emerald-100'}`}
                      >
                        <div className={`absolute left-0 top-0 bottom-0 w-1 ${appt.is_emergency ? 'bg-red-500 animate-pulse' : 'bg-emerald-500'}`} />
                        <div className="flex justify-between items-start pl-2">
                          <div>
                            <h4 className="font-bold text-surface-900">
                              {patient?.name || 'Unknown Patient'} {patient?.display_id && <span className="text-surface-400 font-normal text-xs ml-1">({patient.display_id})</span>}
                            </h4>
                          ...
                          {appt.is_emergency ? (
                            <span className="px-2.5 py-1 rounded-full text-xs font-bold uppercase bg-red-100 text-red-700 animate-pulse flex items-center gap-1.5 border border-red-200">
                              <Activity className="w-3 h-3" /> Emergency
                            </span>
                          ) : (
                        ...
                        <div className="mt-4 pt-3 border-t border-surface-100 pl-2">
                          <Button 
                            ...
                            className="w-full"
                            onClick={async () => {
                              // Mark appointment as complete so they leave the waiting room
                  ...
                  <div className="flex items-start justify-between mb-4 gap-2">
                    ...
                    <div className="shrink-0 max-w-[50%] overflow-hidden">
                      <Badge variant={c.status === 'CLOSED' ? 'success' : c.status === 'DRAFT' ? 'neutral' : 'primary'}>
                        <span className="truncate block">{formatCaseStatus(c.status)}</span>
                      </Badge>
                    </div>
                  </div>
